...
> base::package_version("dplyr")
Error: invalid version specification ‘dplyr’
> base::package_version(dplyr)
Error in base::package_version(dplyr) : object 'dplyr' not found
> package_version(dplyr)
Error in package_version(dplyr) : object 'dplyr' not found
> package_version("dplyr")
Error: invalid version specification ‘dplyr’
...
| Check what version of dplyr you have with packageVersion("dplyr").
...
| Use rm("mydf") to remove the original data frame from your workspace.
...
| Use select(cran, ip_id, package, country) to select only the ip_id, package, and country variables from the cran
| dataset.

> head(select(cran,ip)id)
Error: unexpected symbol in "head(select(cran,ip)id"
> head(select(cran,ip_id))
# A tibble: 6 x 1
  ip_id
  <int>
1     1
...
| Type select(cran, ip_id, package, country) to select only the ip_id, package, and country variables from the
| cran dataset.
...
| negate the entire sequence of numbers from 5 through 20, so that we get -5, -6, -7, ... , -18, -19, -20. Try the
| same thing, except surround 5:20 with parentheses so that R knows we want it to first come up with the sequence
| of numbers, then apply the negative sign to the whole thing.
...
> select(cran , -(1:5))
# A tibble: 225,468 x 6
   r_arch r_os      package      version country ip_id
   <chr>  <chr>     <chr>        <chr>   <chr>   <int>
 1 x86_64 mingw32   htmltools    0.2.4   US          1
 2 x86_64 mingw32   tseries      0.10-32 US          2
 3 x86_64 linux-gnu party        1.0-15  US          3
 4 x86_64 linux-gnu Hmisc        3.14-4  US          3
 5 x86_64 linux-gnu digest       0.6.4   CA          4
 6 x86_64 linux-gnu randomForest 4.6-7   US          3
 7 x86_64 linux-gnu plyr         1.8.1   US          3
 8 x86_64 linux-gnu whisker      0.3-2   US          5
 9 NA     NA        Rcpp         0.10.4  CN          6
10 x86_64 linux-gnu hflights     0.1     US          7
# ... with 225,458 more rows
...
| You got it right!
  ...
  |=====================                       |  47%
| Now that you know how to select a subset of columns using select(), a natural next question is "How do I select
| a subset of rows?" That's where the filter() function comes in.
...
| filter(cran, country == "US" | country == "IN") will return all rows for which country equals "US" or "IN". The
| `|` symbol (called a pipe operator) probably shares a key with the backslash on your keyboard.
...
> 
> filter(size)
Error in filter(size) : object 'size' not found
> filter(cran ,r_os == "linux_gnu")
# A tibble: 0 x 11
# ... with 11 variables: X <int>, date <chr>, time <chr>, size <int>, r_version <chr>, r_arch <chr>, r_os <chr>,
#   package <chr>, version <chr>, country <chr>, ip_id <int>

| Keep trying! Or, type info() for more options.

| This is a tricky one. filter(cran, size > 100500, r_os == "linux-gnu") will give us all rows for which size is
| strictly greater than 100500 and r_os is "linux-gnu".
...
Error: Problem with `filter()` input `..1`.
x Input `..1` must be of size 225468 or 1, not size 11.
i Input `..1` is `!is.na(names(cran))`.
Run `rlang::last_error()` to see where the error occurred.
...
| This is another tricky one. filter(cran, !is.na(r_version)) will return all rows of cran for which r_version is
| NOT NA.
...
> 
> cran2 <- select(size : id_id)
Error in select(size:id_id) : object 'size' not found
> cran2 <- select(cran , size : id_id)
Error: Can't subset columns that don't exist.
x Column `id_id` doesn't exist.
Run `rlang::last_error()` to see where the error occurred.
> cran2 <- select(cran , size : pd_id)
Error: Can't subset columns that don't exist.
x Column `pd_id` doesn't exist.
Run `rlang::last_error()` to see where the error occurred.
...
# A tibble: 225,468 x 11
        X date       time        size r_version r_arch r_os         package      version country ip_id
    <int> <chr>      <chr>      <int> <chr>     <chr>  <chr>        <chr>        <chr>   <chr>   <int>
 1 225464 2014-07-08 23:43:15    5933 NA        NA     NA           CPE          1.4.2   CN      13859
 2 225424 2014-07-08 23:40:17  569241 3.1.0     x86_64 mingw32      multcompView 0.1-5   US      13858
 3 225371 2014-07-08 23:25:46  228444 3.1.0     x86_64 mingw32      tourr        0.5.3   NZ      13857
 4 225368 2014-07-08 23:20:35  308962 3.1.0     x86_64 darwin13.1.0 ctv          0.7-9   CN      13856
 5 225359 2014-07-08 23:13:38  950964 3.0.3     i386   mingw32      knitr        1.6     CA      13855
 6 225360 2014-07-08 23:13:41   80185 3.0.3     i386   mingw32      htmltools    0.2.4   CA      13855
 7 225361 2014-07-08 23:13:44 1431750 3.0.3     i386   mingw32      shiny        0.10.0  CA      13855
 8 225349 2014-07-08 23:40:35 2189695 3.1.0     x86_64 mingw32      RMySQL       0.9-3   US      13854
 9 225290 2014-07-08 23:50:44 4818024 3.1.0     i386   mingw32      igraph       0.7.1   US      13853
10 225220 2014-07-08 23:04:41  197495 3.1.0     x86_64 mingw32      coda         0.16-1  US      13852
# ... with 225,458 more rows

| You almost had it, but not quite. Try again. Or, type info() for more options.

| arrange(cran2, desc(ip_id)) will order the rows of our dataset from largest ip_id to smallest ip_id.
...
Error: Problem with `mutate()` input `size_gb`.
x object 'size_mb' not found
i Input `size_gb` is `size_mb/2^10`.
Run `rlang::last_error()` to see where the error occurred.
> cran3
# A tibble: 225,468 x 3
   ip_id package         size
...
> mutate(cran3 , correct_size = size < 200)
# A tibble: 225,468 x 4
