...
df <- base::with(data,
                 base::Map(read.table,file = fileLoc , 
                                    ...
                                    ,quote = "", comment.char = "")) ## For Each File !!
...
target_indexes <- base::grep("mean\\(\\)|std\\(\\)",
                              x = df$features[[2]])
length(target_indexes)
target_Final_indexes <- c(1,2, target_indexes + 2)
ExtractedData <- Merged[, target_Final_indexes]
ncol(ExtractedData)
...
ExtractedData[[2]] <- base::factor(x = ExtractedData[[2]],
                                   labels = df$activity_labels$V2,
                                   levels = df$activity_labels$V1)
ExtractedData[[1]]
###ENT OF THE NUMBER 3 ###
...
length(Var_names)
tidy_data$V1
names(tidy_data) <- c("subject","activity", Var_names)
names(tidy_data)
## END OF THE NUMBER 4
